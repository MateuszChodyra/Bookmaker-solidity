...
contract Betting is Ownable {
    ...
    event CategoryEdited(uint indexed _id, string _categoryName);
    ...
    function editCategory(uint _id, string memory _categoryName) public onlyOwner {
        categories[_id].name = _categoryName;
        emit CategoryCreated(_id, _categoryName);
    }
  ...
  // }
    ...
    function betMatches(uint[] calldata _matchIds, MatchResult[] calldata _matchResults) public payable returns(uint) {
        ...
        for (i = 0; i < _matchIds.length; i++) {
            newMultiBet.bets.push(Bet(_matchIds[i], _matchResults[i]));
    ...
    function getUserBetsId(address _address) public view returns(uint[] memory) {
        return ownerToBets[_address];
    }

    function getUserBets() public view returns(MultiBet[] memory) {
        MultiBet[] memory userBets = new MultiBet[](ownerToBets[msg.sender].length);
        for (uint i = 0; i < ownerToBets[msg.sender].length; i++) {
            MultiBet storage bet = bets[ownerToBets[msg.sender][i]];
            userBets[i] = bet;
        }
    ...
    function getUserBets(address _address) public view onlyOwner returns(MultiBet[] memory) {
        MultiBet[] memory userBets = new MultiBet[](ownerToBets[_address].length);
        for (uint i = 0; i < ownerToBets[_address].length; i++) {
            MultiBet storage bet = bets[ownerToBets[_address][i]];
            userBets[i] = bet;
        }
